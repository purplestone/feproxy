...
function getName(pathname) {
  return (name => (name ? name : '/'))(pathname.split('/').pop());
}
...
module.exports = async (ctx, next, param) => {
  ...
  if (param.saveHostname) {
    headers.hostname = ctx.hostname;
  }

  let proxy;
  const oReq = {
    hostname: param.hostname || urlInfo.hostname,
    port: param.port || urlInfo.port,
  };
  try {
    proxy = await new Promise((resolve, reject) => {
      const oQ = {
        hostname: oReq.hostname, // 可以通过param修改host
        port: oReq.port,
        path: urlInfo.path,
        headers,
        method,
      ...
      const req = (urlInfo.protocol === 'https:' ? https : http).request(oQ, res => {
        res.on('error', err => {
          console.error('Response error', urlInfo.href, err.code || '');
        });
        req.url = res.url = urlInfo.href;
        req.headers = req.getHeaders();
        // res.headers['redirect-url'] = urlInfo.href;
        res.headers['orgin-url'] = ctx.req.url;
        res.headers['orgin-name'] = getName(url.parse(ctx.req.url).pathname);
        resolve({ req, res });
      });
      req.on('error', err => {
  ...
  ctx.set('redirect-url', urlInfo.href);
