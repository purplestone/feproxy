...
module.exports = async (ctx, next, param) => {
  ...
  if (param.saveHostname) {
    headers.hostname = urlInfo.hostname;
  }
  headers['redirect-url'] = urlInfo.href;
  ...
  try {
    proxy = await new Promise((resolve, reject) => {
      ...
      const req = (urlInfo.protocol === 'https:' ? https : http).request(oQ, res => {
        ...
        console.log('req:', { hostname: oQ.hostname, path: oQ.path, headers: oQ.headers });
        resolve({ req, res });
      });
      req.on('error', err => {
  ...
  proxy.res.headers['orgin-url'] = ctx.req.url;
